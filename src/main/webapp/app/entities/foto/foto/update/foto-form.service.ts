...
type FotoFormGroupContent = {
  ...
  conteudo: FormControl<IFoto['conteudo']>;
  conteudoContentType: FormControl<IFoto['conteudoContentType']>;
};
...
export class FotoFormService {
  createFotoFormGroup(foto: FotoFormGroupInput = { id: null }): FotoFormGroup {
    ...
    return new FormGroup<FotoFormGroupContent>({
      ...
      conteudo: new FormControl(fotoRawValue.conteudo, {
        validators: [Validators.required],
      }),
      conteudoContentType: new FormControl(fotoRawValue.conteudoContentType),
    });
  }
