...
export class DiaSemanaComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (id) {
      this.diaSemanaService
        .findAllByPeriodoId(id, {
          page: pageToLoad - 1,
          size: this.itemsPerPage,
          sort: this.sort(),
        })
        .subscribe({
          next: (res: HttpResponse<IDiaSemana[]>) => {
  ...
  trackId(_index: number, item: IDiaSemana): number {
    return item.id!;
  }

  delete(diaSemana: IDiaSemana): void {
    const modalRef = this.modalService.open(DiaSemanaDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.diaSemana = diaSemana;
  ...
  protected onSuccess(data: IDiaSemana[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
